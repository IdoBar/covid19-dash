...
covid_daily_data <- covid_data_eu %>% 
  ...
  mutate(Total_cases=cumsum(cases), Total_Deaths=cumsum(deaths), Cases_per_mill=Total_cases*1e6/(popData2018),
...
choroplet_data <- covid_daily_data %>%  group_by(Country) %>% arrange(desc(dateRep)) %>% 
  slice(1) %>% filter(Total_cases>0) %>% mutate(log_cases=log(Total_cases), 
                                            ...
                                            Cases_per_mill=scales::number(Cases_per_mill, accuracy = .01,big.mark = ","), 
                                                pop=scales::number(popData2018,big.mark = ","))
...
# plot map
  ...
  # return Highcharts.numberFormat(Math.exp(this.value), 0, '.', ','); }"))) %>%
    hc_tooltip(useHTML=TRUE,headerFormat='',
               pointFormat = '{point.Country} confirmed cases : <span style="     border-radius: 4px; padding-right: 4px; padding-left: 4px; background-color: gold !important;" >{point.Total_cases}</span><br/>Deaths: <span style="     color: white !important;border-radius: 4px; padding-right: 4px; padding-left: 4px; background-color: orangered !important;" >{point.Total_Deaths} (CFR {point.CFR})</span><br/>Population (2018): <b>{point.pop}</b><br/>Cases per million: <b>{point.Cases_per_mill}</b>') %>%
    hc_mapNavigation(enabled = TRUE) %>%
